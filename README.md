...
2. The CNN layer receives the resized picture, and after convolution and pooling, the size is changed after each convolution by adding a pad, and after the pooling is halved, the final feature map is proportional to the input map. Network) and ROI layer reuse 
    + A total of 4 pooling, the number of channels of the final convolution output is 512 (VGG16), the corresponding ratio of the feature map size and the input zoom map mapping is 1/16, the final output of the volume base layer is 'conv5_3', and the input is sent to RPN to calculate The corresponding box is sent all the way to the ROI to calculate the corresponding feature map for classification.
